...
You can also disable this warning by passing {iframes: {ignoreCrossOrigin: true}} to $Window.`));
	...
	function handle_pointer_activation(event) {
		...
		$w.bringToFront();
		// PATCHED: a tap/click directly on a form control focuses it by itself. Don't take focus away to give it
		// back a frame later: iOS Safari ignores programmatic focus() outside the tap, so the keyboard just closed.
		if (/** @type {Element} */ (event.target).closest?.("input, textarea, select, button, [contenteditable=true], [contenteditable='']")) {
			return;
		}
