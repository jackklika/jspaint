...
// sites Worker handles at /~name/x/guestbook); entries live in the site's Durable Object, newest first.
...
<form method="post" action="/~${escape_html(context.site)}/x/guestbook" style="margin:4px 0">
...
<div style="max-height:220px;overflow:auto;text-align:left">${list}</div>`;
	...
	/**
	 * POST /~name/x/guestbook: adds an entry (rate-limited per visitor), then goes back to the page.
	 * @param {{ form: FormData, context: import("./index.js").XElementContext }} input
	 */
	async action({ form, context }) {
		if (String(form.get("website") || "")) {
			return { status: 303, location: String(form.get("back") || `/~${context.site}/`) }; // honeypot: pretend it worked
		}
		...
		let back = String(form.get("back") || "");
		if (!back.startsWith(`/~${context.site}/`) || back.includes("//") || back.includes("..")) {
			back = `/~${context.site}/`;
		}
		return { status: 303, location: `${back}#guestbook` };
	},
};
