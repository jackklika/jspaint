...
- The top of the toolbox is vanilla MS Paint (16 classic tools, untouched); anything new goes below the groove as a page tool, keeping the count even (no empty cell).
...
- Don't commit or push unless asked. Never edit `worker/editor/.secret.txt` or `*.dev.vars` (secrets, gitignored).
- `worker/` holds the two Cloudflare Workers. (The old `agent-server/` dev tool — Code Agent window, Agent Drive — was removed on 2026-09-10; it's in git history if ever wanted back.)
