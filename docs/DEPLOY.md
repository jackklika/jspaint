...
- The admin: `ADMIN_EMAILS` in `worker/editor/wrangler.jsonc` (comma-separated) names the accounts whose Google sign-in acts as the master key and opens `https://edit.coolpaint.world/admin` — every page by site and by account, with hide/delete/lock/quota/report levers (docs/DESIGN.md §9 "The admin"). Locally `.dev.vars` names `admin-test@example.com`, which google-auth.test.mjs signs in as. A taken-down site is a marker in the bucket (`sites/<name>/.moderation.json`); `curl -X POST …/api/admin/sites/<name>/moderate -d '{"disabled":true,"reason":"…"}'` with the master key does the same from a terminal.
