...
- **GifCities** — the GIF picker window (search, grid, click or drag onto the page → sticker); the editor Worker proxies `gifcities.org` (no API, no CORS) and chosen GIFs are copied into the site's `gifs/` on save. **Favorites** (2026-09-13, Jack): a ♥ in the corner of every GIF keeps it in the picker's Favorites tab (`.gif-picker-tab[data-tab=favorites]`, `.gif-heart`); favorites live in this browser (localStorage `jspaint favorite gifs`, by GifCities id, up to 300, newest first) and, signed in with an account, on the account (`favorites` table in the `Accounts` DO; `GET/POST /auth/favorites`, session cookie) — the two are merged (the union, newest first) whenever the picker opens or the sign-in changes (`sync_favorites`), each ♥ posts as it happens, so favorites follow the person between devices and hearts made signed out come along on sign-in; the tab counts them and a favorite adds to the page like any result.
