...
- **The globe** — a 50×50 button at the bottom of the toolbox (a web-1.0 spinning pixel earth): My Site in one click. Signed out it opens Sign In; signed in it shows the site view (address, this page, Browse Files…, Save, Share, Sign Out); a guest sees whose page they're on and Share (2026-09-10, `src/site-button.js`).
- **First run** — the Brush tool, largest round size, dark blue foreground (`#000080`): a shared link lands a newcomer ready to paint.
- **Toolbox — the top stays vanilla.** The top of the toolbox is MS Paint's: the 16 classic tools, same order, same icons, same behavior, nothing added or removed (Jack, 2026-09-10: "identical to the original mspaint"). Everything the site builder adds lives **below the groove** in the page tools (`src/page-tools.js`, 16×16 pixel icons inlined as SVG), kept to an even count so the two-column grid has no empty cell: **Pointer** (select, move, resize, double-click to edit), Text Box, **Web Text** (the Text tool's box, but finishing keeps the words as an editable, linkable text layer; letters-in-a-box icon; the Font toolbar's old "Web" toggle is gone), Divider, GIF (one-shot: opens the GifCities window), Image (one-shot: file → sticker), Table, Colored Box, Guestbook, Visitor Counter, Music, HTML. Element tools work like the Text tool: click, or drag a box. Adding any element switches to the Pointer tool (like Paste switches to Select). Headings are text boxes with a bigger font; scrolling text is the **Marquee toggle in the Font toolbar** (next to B/I/U), which turns the text box being edited into a `<marquee>` and back (2026-09-10).
...
- **Content-Security-Policy on every page**: `default-src 'self'; img-src 'self' data:; media-src 'self'; style-src 'self' 'unsafe-inline'; script-src 'none'` (raise `script-src` only for our own optional live-edit snippet, nonce'd — **never for analytics**: published pages carry no scripts and no trackers, on purpose; product analytics lives on the editor origin only, `worker/shared/analytics.js`). `X-Frame-Options`/`frame-ancestors` so pages can't be framed by the editor for clickjacking, and vice versa.
