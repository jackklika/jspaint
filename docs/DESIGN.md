...
- **The globe** — a 50×50 button at the bottom of the toolbox (a web-1.0 spinning pixel earth): My Site in one click. Signed out it opens Sign In; signed in it shows the site view (address, **Right now: 👁 N viewing (M today) · ✏️ K editing** — viewers are distinct addresses that loaded a page of the site in the last 5 minutes, counted by the sites Worker on each page load into the site's `SiteState` DO and read from `/~name/x/stats.json`; editors are the clients in the site's pages' live rooms, `GET /api/sites/:name/presence` — this page, My Site…, Save, Share, Sign Out); a guest sees whose page they're on and Share (2026-09-10, `src/site-button.js`).
