...
- **The globe** — a 50×50 button at the bottom of the toolbox (a web-1.0 spinning pixel earth): My Site in one click. Signed out it opens Sign In; signed in it shows the site view (address, this page, My Site…, Save, Share, Sign Out); a guest sees whose page they're on and Share (2026-09-10, `src/site-button.js`).
...
- **Cards** (2026-09-12, Jack: richer blog posts, Ghost-style, kept modular and optional; branch `cards`) — `src/cards.js`. Inside a section being edited, **`/` on an empty line** (or the Font toolbar's **Insert**) opens a Win98 menu of cards: Callout (emoji, colored box), Button (a link styled as a button; its address from the link dialog), Toggle (`<details>`: closed on the page, open while editing), Quote, Code, Divider, Picture (with a caption; from the Pictures window), Gallery (a grid of site pictures picked from the Pictures window while the gallery is selected), and the site's own elements (Counter, Guestbook, Music, Folder View, Contents, Last Updated — the sites Worker renders `<x-*>` wherever they are). A card is one element of the section's html, `<… class="card" data-card="kind">`; its typable parts are `.card-text`. Editing attributes (`contenteditable`, a toggle's `open`) are put on at render time and stripped from the model (`normalize_container_html`), so published markup is clean and works with no JS. A card is selected by clicking its chrome (not its text): a **grip** beside it offers drag (to another spot or another section), ↑ ↓, ✕, and the kind's own buttons (width normal/wide/full for pictures and galleries, background for callouts, the link for buttons, settings for `<x-*>`); ↑/↓, Backspace, Enter, and Escape work on a selected card; Escape in a card's text steps out of it first. Card styles ride in `COLLAGE_CSS` (`card_css`), scoped to `.column > .section` on the page and to the editor's section element; the column's geometry travels as `--column-left`/`--page-width` for wide/full cards; `site.css` can override. Sections re-stack when their contents change height (a `ResizeObserver`), `img` in sections gets `max-width: 100%`, and `<x-toc>`/RSS summaries find sections by depth (`worker/shared/sections.js`), whatever the attribute order. Nothing of this shows unless `/` or Insert is used.
