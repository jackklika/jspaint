...
- **Worker `sites`** (the sandbox, §9): `GET /~name/` and `/~name/<path>` → object from R2 `sites/name/…`. Pages are served as-is except that `<x-*>` elements are rendered server-side via the registry (§3.5), backed by a per-site Durable Object (guestbook entries, counter). No JavaScript needed on a published page. **Clean addresses** (2026-09-14): `/~name/about` serves `about.html` (the `.html` address still works); a folder's bare address (`/~name/blog`) redirects to `/~name/blog/` so relative addresses inside resolve. **Custom 404**: a site's `404.html` — a page made in Paint like any other (New Page… named `404`) — is served, with status 404, for any address of that site that isn't a file (x-elements rendered, `site.css` linked, not counted as a view); without one, the default green-on-black 404. The root site's `404.html` is the domain's.
...
- **Page Properties colors** (2026-09-14, Jack) — the Background color and Text color fields each have a swatch button (`.page-properties-swatch`) showing the color (a slash for "empty: white") that opens Paint's own Edit Colors dialog (`choose_color` in edit-colors.js, now exported — the same dialog as Colors › Edit Colors…); a pick lands in the field as `#rrggbb` (`to_hex`), and typing a color recolors the swatch. Test: `page-properties.test.mjs`.

- **Served pages are cached** (2026-09-15): the sites Worker keeps each rendered page in the Cache API under the site's **generation**, a counter in its `SiteState` object that every publish and delete (the editor Worker POSTs `/~name/x/published` after writing or deleting a page, `site.json`, or `site.css`) and every guestbook signing bumps — a new generation is a new key, so nothing is ever stale and nothing needs purging. A view is one trip to the site's object (`view()`: the visit recorded, the generation and the page's hit count back), then the cached copy, before R2 is asked at all; a miss renders (sanitize, `<x-*>`, stylesheet) and stores for an hour. The visitor counter can't live in cached HTML, so it renders as a slot (`<span data-x-counter>`; `count_slot` in the element context) that HTMLRewriter fills with the count on every view, and the visit is added after the answer. HEAD and 404s aren't visits; every address of a page shares one copy; the site's `404.html` is cached like any page; visitors still get `no-cache, no-transform`. `X-Cache: hit|miss` says which happened. Anyone may POST `/x/published` (the sites Worker keeps no secrets); it only costs a re-render, and MALICIOUS_ACTOR_PLAN.md phase 0 rate-limits it.
- **Server errors reach PostHog** (2026-09-14, Jack: "make sure that this kind of error would be a posthog $exception event", after the quota outage) — `worker/shared/exceptions.js` `capture_exception(env, ctx, error, props)` sends a `$exception` event with `$exception_list` (type, value; `error_kind: "server"`, `worker`, `route`, `status`, `code`) straight to PostHog's ingestion with the Worker's `POSTHOG_API_KEY`, at most once a minute per distinct failure per isolate. Called from the editor Worker's top-level catch (which also turns the Durable Objects quota error into a 503 `storage-quota`), from `PageRoom.webSocketMessage` (a failing room message now answers `{type:"error"}` to the client instead of dropping the socket), and from the sites Worker's new top-level catch (a "Back soon" 503 page for the quota). The sites Worker has no `POSTHOG_API_KEY` yet: set one (`wrangler secret put POSTHOG_API_KEY -c sites/wrangler.jsonc`) and its failures report too. The browser's own `$exception` funnel (`src/error-handling-enhanced.js` → `app-analytics.js`) is untouched. **People never see the detail** (2026-09-14, Jack): a 5xx answers with one plain sentence and a `code` (`storage-quota`, `server-error`), the live room's error messages say the same, the sites Worker's failure page is a plain "Back soon"; on the client `request_error(status, server_message)` (site-publish.js, used by `api()` and the publish uploads) shows the server's own words only for a 4xx the person can act on, the plain sentence (`SERVER_ERROR_TEXT`) for a 5xx or no answer, and puts the detail in the console. Test: `server-errors.test.mjs` (stubbed 503/500/409). **Everything a person felt reaches Error tracking**: on the client, `report_error(kind, error)` (site-publish.js) hands failed requests (`request`) and every error dialog that carries an error (`show_error_message` → `dialog`) to the app's own funnel (`app-analytics.js` `track_app_error` → `posthog.captureException`), best effort; on the server, `capture_exception` builds `$exception_list` with the stack as raw frames, returns PostHog's status, and never throttles a `test` event. **Proving the pipeline**: `POST /api/debug/exception` with the master key sends a test exception and answers `{ reported, posthog_status, configured }` (docs/DEPLOY.md). The sites Worker reports only once it has a `POSTHOG_API_KEY` secret. Tests: `exceptions.test.mjs` (the payload, the key, the repeat guard, in Node), `server-errors.test.mjs` (a PostHog stand-in sees the request failure and the dialog).

- **Select Elements** (2026-09-14, Jack: "a separate select tool for html elements, similar to the mspaint select") — the toolbox tool right after the Pointer (`src/element-selection.js`, `TOOL_SELECT_ELEMENTS` in page-tools.js). It's a select-box tool like Paint's Select, so tools.js draws the dashed drag rectangle; on release every element the box touches — text boxes, pictures, web text; sections stay in their column and aren't grouped — becomes the group (`.multi-selected` on each, a dashed `.element-group` box with "N selected"); an element the whole box sits inside is left out when the box touches anything else (a page-wide picture behind everything). Shift adds (a box or a click); a box over one element hands it to the ordinary selection; a click on nothing, Escape, or another tool lets go; Ctrl+A takes every element. The group moves as one — a drag on any member (a capture-phase pointerdown on the canvas area, ahead of the element's own handler) or the arrow keys, one "Move Elements" step per gesture (`move_group`, keyed like the single-element moves) — and Delete removes it as one "Delete Elements" step (`remove_*_by_id` inside one `undoable`). Undo recreates the elements; the group keeps its ids and re-marks what still exists. `select_tools` now announces `tool-changed`. **Page Style** joined the toolbox as the sixteenth page tool (even columns): it opens Page Properties. Test: `element-select.test.mjs`.

- **Undo across pixels and elements** (2026-09-14, Jack: "ctrl+z doesn't work well when undoing between mspaint elements and html elements") — element moves, resizes and arrow-key nudges (blocks, stickers, web text) are ordinary history steps, one per action: a drag's pointer moves and a run of nudges of the *same* element fold into one step (`move_undoable` in blocks.js/stickers.js/text-layers.js keeps the last step and the element's id; another element's move starts a new step), and Ctrl+Z / Ctrl+Y stop at each. They used to be `soft` steps — upstream's flag for the Text tool's in-between states, which undo *skips* — so one Ctrl+Z after "move, then paint" reverted both. The one deliberately soft element step left is re-editing web text (the layer gone, the text box open). Test: `undo-steps.test.mjs`.
...
- **Editor start & first run** (2026-09-14, Jack: "edit this site" should go to `edit.coolpaint.world/~root/`, and `edit.coolpaint.world/` should be where someone new makes a site) — three front doors, decided in `open_site_from_url_inner` (my-site.js): **`/~name[/page]`** (the Worker 302s to `/?site=&page=`) opens the page — the site's own page when you're signed in as it, a copy otherwise (§9); **`/`** opens your site's front page when signed in, and for anyone else the **starter page** with the **Welcome window**; **`/new`** (302 to `/?new=1`) opens the starter page for anyone, signed in or not (a second site is New Site… on the globe). The starter (`open_starter_page`) is the root site's `welcome.html` when it has one — the root site's owner draws it in Paint at `edit.<domain>/welcome`, like any page — else a built-in start (`new_site_page(path, { starter: true })`: a heading, a section, a visitor counter), opened as a *fresh* page (`{ site_page: "index.html", fresh: true }`, never `copy_of`), so Save puts it on a site of your own. The Welcome (`src/welcome.js`, `.welcome-window`) says what this is and what saving does, offers Google and the site's homepage, and has "Show this next time" (localStorage `jspaint welcome seen`). **Sign-in keeps the drawing**: a Save that has to sign in passes `resume` to the Sign In dialog, whose Google button goes to `/auth/google?next=/?signed_in=1&resume=save&page=<path>#local:<id>` (`google_sign_in_url`; auth.js `safe_next` keeps the query and the hash) — the Google button first flushes the session's backup (`flush_session_backup`, sessions.js — the debounced autosave may not have run since the last stroke), back from Google the same session restores (the sidecar now keeps `fresh` too), `page_restored` waits for it, and `save_page_to_site` carries on: the account's site, or the New Site form ("Your page is saved to it right after") and then the publish. The homepage's links are Jack's to point: "edit this site" → `/~root/`, "Create your own site!" → `/new`.
- **GifCities** — the GIF picker window (search, grid, click or drag onto the page → sticker); the editor Worker proxies `gifcities.org` (no API, no CORS) and chosen GIFs are copied into the site's `gifs/` on save. **Favorites** (2026-09-13, Jack): a ♥ in the corner of every GIF keeps it in the picker's Favorites tab (`.gif-picker-tab[data-tab=favorites]`, `.gif-heart`); favorites live in this browser (localStorage `jspaint favorite gifs`, up to 300, newest first; a favorite is `source:id` — `gifcities:<ID>` today; a new store (our own GIF library) is one more entry in `GIF_SOURCES` in gif-picker.js, how to make a URL from an id and how to recognize its URLs, and the server accepts any `source:id`) and, signed in with an account, on the account (`favorites` table in the `Accounts` DO; `GET/POST /auth/favorites`, session cookie) — the two are merged (the union, newest first) whenever the picker opens or the sign-in changes (`sync_favorites`), each ♥ posts as it happens, so favorites follow the person between devices and hearts made signed out come along on sign-in; the tab counts them and a favorite adds to the page like any result.
...
- **Sessions and the claims cookie** (2026-09-14): a Google sign-in makes a 90-day session (a random token, stored as its SHA-256 in `Accounts`) and, beside it, an hour-long **claims cookie** `coolpaint_id` = `v1.<claims>.<HMAC-SHA256>` naming the user, the session's hash, and the sites they own. A request carrying a valid one is known without a trip to the single global `Accounts` object (every API call and room join used to make two); when it's missing or stale, the session is looked up as before and a fresh claims cookie rides back on the response (`worker/editor/auth.js` `session_of`, `with_refreshed_claims`). Claiming a site refreshes it at once; a site claimed elsewhere is still asked about. Routes that need the account's email or a sign-out to show at once ask for a fresh lookup (`/api/whoami`, `/auth/sites`, sign-out). Signing out deletes the session and clears both cookies; a kept claims cookie outlives that by at most its hour — the price of not asking. The signing key is `SESSION_SIGNING_KEY` when set (rotating it only costs everyone one lookup), else one derived from the master key.
- **Anyone may open any published page in Paint** (`edit.<domain>/~name/page.html`): it opens as a copy, read from the public files API, with no room and no credentials; publishing it back to that address needs that site's password — the copy goes to whatever site you sign in to. A copy (like a fresh page) named like a page you already have asks before replacing it (2026-09-14), and its `<x-*>` elements preview from the copied site, so it looks like the live page.
