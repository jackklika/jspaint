...
assert.equal(await entry("/~yourname"), "302 /?site=yourname");
assert.equal(await entry("/~yourname/"), "302 /?site=yourname");
assert.equal(await entry("/~yourname/about.html"), "302 /?site=yourname&page=about.html");
assert.equal(await entry("/~yourname/blog/"), "302 /?site=yourname&page=blog%2Findex.html"); // cspell:disable-line
assert.equal(await entry("/~yourname/gifs/x.gif"), "302 /?site=yourname", "not a page: just the site");
assert.equal(await entry("/~root"), "302 /?site=root");
assert.equal(await entry("/~yourname/about"), "302 /?site=yourname&page=about.html", ".html is optional");
assert.equal(await entry("/~Your Name!"), "404");
