...
const keys = ids.map((id) => `gifcities:${id}`); // (a favorite is source:id, so other stores can join later)
...
await page.click(`.gif-tile[data-gif="${keys[0]}"] .gif-heart`);
await page.click(`.gif-tile[data-gif="${keys[2]}"] .gif-heart`);
assert.deepEqual(await page.$$eval(".gif-picker-panel[data-tab=search] .gif-heart", (els) => els.map((el) => el.textContent)), ["♥", "♡", "♥"]);
...
assert.deepEqual(JSON.parse(await page.evaluate(() => localStorage.getItem("jspaint favorite gifs"))).map((f) => f.id), [keys[2], keys[0]], "newest first, as source:id");
...
assert.deepEqual(await page.$$eval(".gif-picker-favorites .gif-tile", (els) => els.map((el) => el.dataset.gif)), [keys[2], keys[0]]);
assert.equal(await page.$eval(".gif-picker-panel[data-tab=search]", (el) => el.offsetParent === null), true, "the search panel is hidden");
await page.click(`.gif-picker-favorites .gif-tile[data-gif="${keys[2]}"] .gif-heart`);
...
await page.click(`.gif-picker-favorites .gif-tile[data-gif="${keys[0]}"]`);
...
// Favorites survive a reload (localStorage) — one saved before stores were named (a bare GifCities id) counts too;
// none → the tab says so
await page.evaluate((id) => { const list = JSON.parse(localStorage.getItem("jspaint favorite gifs")); list.push({ id, width: 24, height: 24, at: 1 }); localStorage.setItem("jspaint favorite gifs", JSON.stringify(list)); }, ids[1]);
...
await page.click(".gif-picker-tab[data-tab=favorites]");
await page.waitForFunction(() => document.querySelectorAll(".gif-picker-favorites .gif-tile").length === 2, null, { timeout: 5000 });
assert.deepEqual(await page.$$eval(".gif-picker-favorites .gif-tile", (els) => els.map((el) => el.dataset.gif)), [keys[0], keys[1]], "the old bare id shows as gifcities:…");
await page.click(`.gif-picker-favorites .gif-tile[data-gif="${keys[0]}"] .gif-heart`);
await page.click(`.gif-picker-favorites .gif-tile[data-gif="${keys[1]}"] .gif-heart`);
